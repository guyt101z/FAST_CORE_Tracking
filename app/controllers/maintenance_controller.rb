class MaintenanceController < ApplicationController
  ...
  def delete
    task = MaintenanceTask.find(params[:id])
    device_id = task.device_id if task
    unless session[:is_admin]
      flash[:error] = 'Only administrators can delete maintenance tasks'
      return redirect_to(:controller => 'reports', :action => 'maintenance', :id => device_id)
    end
      
    if request.post?
      task.destroy
      flash[:success] = task.description + ' was deleted successfully'
      redirect_to(:controller => 'reports', :action => 'maintenance', :id => device_id)
    end
  end
end
