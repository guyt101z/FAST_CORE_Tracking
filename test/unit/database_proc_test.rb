...
class DatabaseProcTest < ActiveSupport::TestCase
  ...
  def test_idle_insert
    ...
    IdleEvent.delete_all
    Reading.delete_all
    insert_idle(2.2, 2.3, now, devices(:device1).imei)
    insert_idle(2.2,2.3, now-100, devices(:device1).imei) #insert out of order duplicate event
    idle_events = IdleEvent.find :all
